...
class LanguageController extends Controller
{
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'build_from' => 'nullable',
            'flag_path' => 'required|image|mimes:jpeg,png,jpg,svg|max:2048',
        ]);
        if ($request->hasFile('flag_path')) {
            $data['flag_path'] = upload_file($request, 'flag_path', 'language/flag');
        }
    ...
    public function update(Language $language, Request $request)
    {
        $data = $request->validate([
            ...
            'status' => 'required|boolean',
            'build_from' => 'nullable',
            'flag_path' => 'nullable|image|mimes:jpeg,png,jpg,svg|max:2048',
        ]);
        if ($request->hasFile('flag_path')) {
            $data['flag_path'] = upload_file($request, 'flag_path', 'language/flag', $request->flag_path);
        }
        $data['code'] = Localizer::formatKey($data['code']);
