 <form action="{{ config('theme.update') ?? route(config('theme.rprefix') . '.store') }}" method="POST"
     ...
     <div class="modal-body text-capitalize">
         ...
         <div class="my-2">
             <label for="flag_path" class="fw-bold ">
                 {{ _localize('flag') }}
                 @isset($item)
                     <span class="text-danger">*</span>
                 @endisset
             </label>
             <input type="file" class="form-control" name="flag_path" id="flag_path"
                 onchange="get_img_url(this, '#flag_url');" @isset($item) required  @endisset
                 accept=".jpeg,.png,.jpg,.svg">
             <img id="flag_url" src="{{ isset($item) ? $item->flag_url : '' }}" width="120px" class="mt-1">
         </div>
         <div class="my-2">
             <label for="code" class="fw-bold ">
